...
namespace SelectRandomFileExtension
{
    // To debug or install DLL, launch "ServerManager.exe" in ""SelectRandomFileExtension\packages\SharpShellTools.2.2.0.0\lib\ServerManager.exe""
    [ComVisible(true)]
    ...
    public class SelectRandomFileExtension : SharpContextMenu
    {
        ...
        protected override ContextMenuStrip CreateMenu()
        {
            ...
            //  Create a 'Select a Random File Here' item
            var itemSelectFile = new ToolStripMenuItem
            {
                Text = "Select a Random File"
            };

            //  When we click, we'll call the 'SelectRandomFile' function
            itemSelectFile.Click += (sender, args) => SelectRandomFile();

            //  Add the item to the context menu
            menu.Items.Add(itemSelectFile);
        ...
        private void SelectRandomFile()
        {
            ...
            //MessageBox.Show(string.Join(", ", selectedPaths));
            if (selectedPaths == null || selectedPaths.Length == 0)
            {
                try
                {
                    string selected_final = RandomlySelect(FolderPath);
                    //MessageBox.Show(selected_final);
                    GoToAndSelectFileInExplorer(selected_final);
                }
                catch { }
            }
            else
            {
                try
                {
                    // Randomly select a file or subdir
                    string selected = selectedPaths[new Random().Next(0, selectedPaths.Length)];
                    string selected_final = RandomlySelect(selected);
                    //MessageBox.Show(selected_final);
                    GoToAndSelectFileInExplorer(selected_final);
                }
                catch { }
        ...
        private string RandomlySelect(string path)
        {
            try
            {
                // Check whether path is a file or dir
                FileAttributes attr = File.GetAttributes(path);
                if (attr.HasFlag(FileAttributes.Directory))
                {
                    // Get all files and subdirs
                    string[] total = GetFilesAndSubdirs(path);
                    //MessageBox.Show(total.Length.ToString());

                    // Randomly select a file or subdir
                    string selected = total[new Random().Next(0, total.Length)];

                    return RandomlySelect(selected);
                }
                else
                {
                    return path;
        ...
        private string[] GetFilesAndSubdirs(string path)
        {
            string[] fileEntries = Directory.GetFiles(path);
            string[] subdirectoryEntries = Directory.GetDirectories(path);
            return fileEntries.Concat(subdirectoryEntries).ToArray();
        }
    }
}
